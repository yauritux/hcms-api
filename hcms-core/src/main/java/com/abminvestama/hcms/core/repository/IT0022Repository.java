...
public interface IT0022Repository extends CrudRepository<IT0022, ITCompositeKeys> {
	...
	@Query("FROM IT0022 it0022 WHERE it0022.pernr = :pernr AND it0022.id.subty = :subty AND it0022.id.endda = :endda AND it0022.id.begda = :begda")
	...
	@Query("FROM IT0022 it0022 WHERE it0022.pernr = :pernr ORDER BY it0022.id.subty ASC, it0022.id.endda DESC")
	Collection<IT0022> findByPernr(@Param("pernr") Long pernr);
	
	@Query("FROM IT0022 it0022 WHERE it0022.pernr = :pernr AND it0022.id.subty = :subty ORDER BY it0022.id.endda DESC")
